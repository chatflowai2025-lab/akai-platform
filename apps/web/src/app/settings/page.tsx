...
const PLAN_LABELS: Record<string, { label: string; color: string }> = {
  ...
  starter: { label: 'Starter — $199/mo', color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
  growth: { label: 'Growth — $599/mo', color: 'text-purple-400 bg-purple-500/10 border-purple-500/20' },
  agency: { label: 'Agency — $1,200/mo', color: 'text-[#D4AF37] bg-[#D4AF37]/10 border-[#D4AF37]/20' },
  scale: { label: 'Agency — $1,200/mo', color: 'text-[#D4AF37] bg-[#D4AF37]/10 border-[#D4AF37]/20' },
};
...
export default function SettingsPage() {
  ...
  const saveBizProfile = async () => {
    ...
    try {
      ...
      setDoc(doc(db, 'users', user.uid, 'voiceConfig', 'config'), { // schema-drift-ok: voiceConfig subcollection write — raw path intentional, no schema helper for subcollection doc refs (FIRESTORE_SCHEMA.md)
        businessName: bizForm.businessName,
        industry: bizForm.industry,
        location: bizForm.location,
    ...
    <DashboardLayout>
      ...
      <div className="space-y-6 max-w-2xl w-full">
            ...
            <div className="relative flex-shrink-0">
              ...
              {avatarUploading && (
                <div className="absolute inset-0 bg-black/60 rounded-full flex items-center justify-center">
                  <div role="status" aria-label="Loading" className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                </div>
              )}
            </div>
        ...
        <Section title="Business Profile">
          ...
          >
            {bizSaving ? (
              <><span role="status" aria-label="Loading" className="w-3.5 h-3.5 border-2 border-black border-t-transparent rounded-full animate-spin" /> Saving…</>
            ) : (
        ...
        <Section title="Plan">
          ...
          <div className="grid grid-cols-3 gap-3 pt-1">
            {[
              { tier: 'starter', label: 'Starter', price: '$199/mo' },
              { tier: 'growth', label: 'Growth', price: '$599/mo' },
              { tier: 'agency', label: 'Agency', price: '$1,200/mo' },
            ].map(p => (
        ...
        <Section title="Notification Preferences">
          ...
          >
            {notifSaving ? (
              <><span role="status" aria-label="Loading" className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Saving…</>
            ) : notifSaved ? (
              '✓ Saved!'
            ) : (
              'Save Notifications'
      ...
      <Section title="Refer a Friend">
        ...
        {referral?.code ? (
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <span role="status" aria-label="Loading" className="w-3.5 h-3.5 border-2 border-gray-600 border-t-transparent rounded-full animate-spin" />
