...
export default function TrialBanner({ trial }: TrialBannerProps) {
  ...
  if (trial.status === 'active') {
    return (
      <div className="flex-shrink-0 bg-[#D4AF37]/10 border-b border-[#D4AF37]/20 px-6 py-2 flex items-center gap-3">
  ...
  if (trial.status === 'ending') {
    return (
      <div className="flex-shrink-0 bg-amber-500/15 border-b border-amber-500/30 px-6 py-2.5 flex items-center gap-3">
        <span className="relative flex h-2 w-2">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75" />
          <span className="relative inline-flex rounded-full h-2 w-2 bg-amber-400" />
        </span>
        <span className="text-amber-300 text-xs font-bold">
          Your trial ends in {trial.daysLeft} day{trial.daysLeft !== 1 ? 's' : ''} remaining
        </span>
      </div>
    );
  }
  ...
  if (trial.status === 'expired') {
    return (
      <div className="flex-shrink-0 bg-[#1a0000] border-b border-red-500/40 px-6 py-3 flex items-center gap-3">
        <p className="text-red-400 text-sm font-black">⛔ Your trial has ended — contact us to continue</p>
      </div>
    );
  }
